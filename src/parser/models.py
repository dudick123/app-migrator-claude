...
class ParseResult(BaseModel):
    ...
    output_path: str | None = Field(default=None, description="Path to output JSON file if successful")
...
class ArgoCDMetadata(BaseModel):
    ...
    namespace: str = Field(default="argocd", description="Namespace where Application resource is created")
...
class ArgoCDSource(BaseModel):
    ...
    repoURL: RequiredStr = Field(description="Git or Helm repository URL")
    targetRevision: str = Field(default="HEAD", description="Git branch, tag, commit, or Helm chart version")
    path: str | None = Field(default=None, description="Directory path within Git repository")
...
class ArgoCDSyncPolicy(BaseModel):
    ...
    automated: dict[str, Any] | None = Field(default=None, description="Automated sync configuration")
    syncOptions: list[str] = Field(default_factory=list, description="Sync options")
...
class ArgoCDSpec(BaseModel):
    ...
    syncPolicy: ArgoCDSyncPolicy | None = Field(default=None, description="Automated sync policy")
    ignoreDifferences: list[dict[str, Any]] = Field(
        default_factory=list,
...
class OutputSource(BaseModel):
    ...
    repoURL: str = Field(description="Git or Helm repository URL")
    revision: str = Field(description="Git branch/tag/commit or Helm version")
    manifestPath: str | None = Field(default=None, description="Directory path within repository")
...
class OutputDestination(BaseModel):
    ...
    clusterName: str = Field(description="Target cluster name")
